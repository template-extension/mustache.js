...
function changeForOS(command) {
  ...
  if(process.platform === 'win32') {
    return command
      .replace(/bin\/tmplat-mustache/g, 'node bin\\tmplat-mustache')
...
describe('Mustache CLI', function () {
  ...
  it('writes syntax hints into stderr when runned with wrong number of arguments', function(done) {
    exec('bin/tmplat-mustache', function(err, stdout, stderr) {
  ...
  it('writes hints about JSON parsing errors when given invalid JSON', function(done) {
    exec('echo {name:"lebron"} | bin/tmplat-mustache - test/_files/cli.mustache', function(err, stdout, stderr) {
  ...
  it('writes module version into stdout when runned with --version', function(done){
    exec('bin/tmplat-mustache --version', function(err, stdout, stderr) {
  ...
  it('writes module version into stdout when runned with -v', function(done){
    exec('bin/tmplat-mustache -v', function(err, stdout, stderr) {
      assert.notEqual(stdout.indexOf(moduleVersion), -1);
      done();
    });
  });
  ...
  describe("without partials", function(){
    ...
    it('writes rendered template into stdout when successfull', function(done) {
      exec('bin/tmplat-mustache test/_files/cli.json test/_files/cli.mustache', function(err, stdout, stderr) {
    ...
    it('writes rendered template into the file specified by the third argument', function(done) {
      ...
      exec('bin/tmplat-mustache test/_files/cli.json test/_files/cli.mustache ' + outputFile, function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, '');
    ...
    it('reads view data from stdin when first argument equals "-"', function(done){
      exec('cat test/_files/cli.json | bin/tmplat-mustache - test/_files/cli.mustache', function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, expectedOutput);
        done();
    ...
    it('writes it couldnt find template into stderr when second argument doesnt resolve to a file', function(done) {
      exec('bin/tmplat-mustache test/_files/cli.json test/_files/non-existing-template.mustache', function(err, stdout, stderr) {
    ...
    it('writes it couldnt find view into stderr when first argument doesnt resolve to a file', function(done) {
      exec('bin/tmplat-mustache test/_files/non-existing-view.json test/_files/cli.mustache', function(err, stdout, stderr) {
  ...
  describe("with partials", function(){
    ...
    it('writes rendered template with partials into stdout', function(done) {
      exec('bin/tmplat-mustache test/_files/cli_with_partials.json test/_files/cli_with_partials.mustache -p test/_files/cli.mustache -p test/_files/comments.mustache', function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, expectedOutput);
        done();
    ...
    it('writes rendered template with partials when partials args before required args', function(done) {
      exec('bin/tmplat-mustache -p test/_files/cli.mustache -p test/_files/comments.mustache test/_files/cli_with_partials.json test/_files/cli_with_partials.mustache', function(err, stdout, stderr) {
        assert.equal(err, null);
        assert.equal(stderr, '');
        assert.equal(stdout, expectedOutput);
        done();
